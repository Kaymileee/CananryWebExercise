      ...
      <div className="grid grid-cols-3 gap-x-5">
        {dataDummies.map((item, index) => (
          <div className="relative rounded-lg cursor-pointer item " key={index}>
            ...
            <div className="absolute bottom-0 left-0 z-50 w-full p-5 font-medium text-white bg-black bg-opacity-90 hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-purple-400 hover:to-pink-600">
              {item.title}
            </div>
          </div>
        ))}
