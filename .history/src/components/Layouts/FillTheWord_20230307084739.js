...
const FillTheWord = () => {
  ...
  let answersIdOne = quesIdOne.map((item) => {
    // console.log(item.DapAn);
    arr.push(item.DapAn.toLowerCase());
  });
  ...
  const handleCheck = () => {
    if (
      ...
      const arrRest = answerUser.filter((item, index) => item !== arr[index]);
      console.log(arrRest.length);
      console.log(arr);
      console.log(100 - (arrRest.length / arr.length) * 100);
      setScore((arrRest.length / arr.length) * 100);
    } else {
      setScore(100);
    }
  };
