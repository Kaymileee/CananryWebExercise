...
const FillTheWord = () => {
  ...
  const handleNext = () => {
    ...
    inpRefTwo.current.value = "";
    inpRefThree.current.value = "";
    inpRefFour.current.value = "";
    inpRefFive.current.value = "";

    setScore(0);
  };
  const inpRef = useRef();
  const inpRefTwo = useRef();
  const inpRefThree = useRef();
  const inpRefFour = useRef();
  const inpRefFive = useRef();
  console.log(quesIdOne[0]);
  return (
    <>
      <DashBoard>
